...
def graph(slot,referent,witness,predicate,recovery,restored_form=None):
    ...
    }],"scope":{},"discourse":{},"realization":{"restored_form":restored_form}}
                    ...
                    for zi,z in enumerate(s.get("ZA",[])):
                        ...
                        if fam=="NIKL_ZA_LEGACY_2020_2024":
                            for ai,a in enumerate(z.get("antecedent",[])):
                                ...
                                rid=f"{sid}|ZA{zi}|A{ai}"
                                yield rid,fam,graph(norm_slot(a.get("type")),w,w,pred,rec,a.get("form"))
                        elif fam=="NIKL_ZA_ELLIPSIS_2025":
                            for ei,e in enumerate(z.get("ellipsis",[])):
                                r=e.get("restored") or {};a=e.get("antecedent") or {}
                                w=anchor(a);rec="ZERO_ANAPHORIC" if w else "ZERO_GENERIC_OR_EXOPHORIC"
                                rid=f"{sid}|ZA{zi}|E{ei}"
                                yield rid,fam,graph(norm_slot(r.get("type")),w,w,pred,rec,r.get("form"))
